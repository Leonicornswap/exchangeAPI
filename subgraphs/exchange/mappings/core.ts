...
import { updatePairDayData, updateTokenDayData, updateLeonicornDayData, updatePairHourData } from "./dayUpdates";
...
export function handleSync(event: Sync): void {
  ...
  let leonicorn = LeonicornFactory.load(FACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  leonicorn.totalLiquidityBNB = leonicorn.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  leonicorn.totalLiquidityBNB = leonicorn.totalLiquidityBNB.plus(trackedLiquidityBNB);
  leonicorn.totalLiquidityUSD = leonicorn.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  leonicorn.totalTransactions = leonicorn.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let leonicorn = LeonicornFactory.load(FACTORY_ADDRESS);
  leonicorn.totalVolumeUSD = leonicorn.totalVolumeUSD.plus(trackedAmountUSD);
  leonicorn.totalVolumeBNB = leonicorn.totalVolumeBNB.plus(trackedAmountBNB);
  leonicorn.untrackedVolumeUSD = leonicorn.untrackedVolumeUSD.plus(derivedAmountUSD);
  leonicorn.totalTransactions = leonicorn.totalTransactions.plus(ONE_BI);
  ...
  let leonicornDayData = updateLeonicornDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  leonicornDayData.dailyVolumeUSD = leonicornDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  leonicornDayData.dailyVolumeBNB = leonicornDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  leonicornDayData.dailyVolumeUntracked = leonicornDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  leonicornDayData.save();
